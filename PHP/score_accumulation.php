...
$session_team_id = (int)$_SESSION['team_id'];

// =======================
// 필터 기본값 설정
// =======================

// 포지션: POST 값이 있으면 그걸 쓰고, 없으면 '0'(전체)
$position_id_state = isset($_POST['position']) ? $_POST['position'] : '0';

// 팀: POST 값이 있으면 그걸 쓰고, 없으면 로그인한 사람의 팀 ID를 기본값으로 사용
if (isset($_POST['team'])) {
    $team_id_state = $_POST['team'];
} else {
    // 세션 team_id 가 1000,2000,... 형태라면 그대로 문자열로
    $team_id_state = (string)$session_team_id;
}
...
// =======================
// '분석 실행' 버튼 눌렀을 때만 쿼리 실행
// =======================
if ($_SERVER["REQUEST_METHOD"] === "POST") {

    $position_id = (int)$position_id_state;
    $team_id     = (int)$team_id_state;

    // ★★★ 팀원이 만든 핵심 SQL 그대로 유지 ★★★
    $sql = "
        SELECT
            p.player_name,
            pp.position_Name,
...
<head>
    ...
    <title>[Windowing/Aggregate] 라운드별 누적 득점</title>
...
<div class="container">
    <!-- 대시보드와 동일 스타일의 네비게이션 (경로만 PHP로 정리) -->
            ...
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><?= number_format($row['points_1_2']) ?></td>
                        <td><?= number_format($row['points_3_4']) ?></td>
                        <td><?= number_format($row['points_5_6']) ?></td>
                        <td><?= number_format($row['cumulative_2']) ?></td>
                        <td><?= number_format($row['cumulative_4']) ?></td>
                        <td><?= number_format($row['cumulative_6']) ?></td>
                    </tr>
                    <?php
                }
            } else {
